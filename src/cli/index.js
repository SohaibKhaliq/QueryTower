...
program
	.name('./QueryTower')
	.description('Welcome to QueryTower')
...
program
	.command('start')
	.description('Start the QueryTower server')
	.action(() => {
...
program
	...
	.description('Start the QueryTower server and view the live output log')
	.action(() => {
...
program
	...
	.description('Start QueryTower in verbose development mode')
	.action(() => {
...
program
	.command('stop')
	.description('Stop the QueryTower server')
	.action(() => {
		require('./running').stop(program.opts());
	});
program
	.command('restart')
	.description('Restart the QueryTower server')
	.action(() => {
		require('./running').restart(program.opts());
	});
program
	.command('status')
	.description('Check the running status of the QueryTower server')
	.action(() => {
...
program
	...
	.description('Run the QueryTower setup script, or setup with an initial config')
...
program
	...
	.description('Launch the QueryTower web installer for configuration setup or install a plugin')
	.option('-f, --force', 'Force plugin installation even if it may be incompatible with currently installed QueryTower')
...
program
	...
	.description('Activate a plugin for the next startup of QueryTower')
	.action((plugin) => {
...
program
	...
	.description('Outputs the most recent administrative events recorded by QueryTower')
	.action((count) => {
...
program
	...
	.description('Run QuertTower upgrade scripts and ensure packages are up-to-date, or run a particular upgrade script')
	...
	.option('-s, --schema', 'Update QueryTower data store schema', false)
	...
	.on('--help', () => {
		console.log(`\n${[
			...
			'\nExamples:',
			`  Only package and dependency updates: ${chalk.yellow('./QueryTower upgrade -mi')}`,
			`  Only database update: ${chalk.yellow('./QueryTower upgrade -s')}`,
		].join('\n')}`);
	})
