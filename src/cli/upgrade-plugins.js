...
async function getSuggestedModules(nbbVersion, toCheck) {
	...
	if (!response.ok) {
		throw new Error(`Unable to get suggested module for QueryTower(${nbbVersion}) ${toCheck.join(',')}`);
	}
...
async function upgradePlugins() {
	try {
		...
		if (['y', 'Y', 'yes', 'YES'].includes(result.upgrade)) {
			...
			console.log(`${chalk.yellow('Package upgrades skipped')}. Check for upgrades at any time by running "${chalk.green('./QueryTower upgrade -p')}".`);
		}
	} catch (err) {
