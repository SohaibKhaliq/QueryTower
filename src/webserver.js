...
server.on('error', (err) => {
	if (err.code === 'EADDRINUSE') {
		winston.error(`QueryTower address in use, exiting...\n${err.stack}`);
	} else {
...
exports.listen = async function () {
	...
	helpers.register();
	logger.init(app);
	await initializeQuerytower();
	winston.info('🎉 QueryTower Ready');
...
async function initializeQuerytower() {
...
async function listen() {
	...
	if (Array.isArray(port)) {
		...
		winston.warn('[startup] If you want to start QueryTower on multiple ports please use loader.js');
	...
	if (isSocket) {
		...
		try {
			...
			winston.error(`[startup] QueryTower was unable to secure domain socket access (${socketPath})\n${err.stack}`);
			throw err;
		}
	}
	...
	return new Promise((resolve, reject) => {
		server.listen(...args.concat([function (err) {
			...
			if (err) {
				winston.error(`[startup] QueryTower was unable to listen on: ${chalk.yellow(onText)}`);
				reject(err);
			}

			winston.info(`📡 QueryTower is now listening on: ${chalk.yellow(onText)}`);
