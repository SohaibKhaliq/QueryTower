...
Plugins.reload = async function () {
	...
	if (Plugins.versionWarning.length && nconf.get('isPrimary')) {
		console.log('');
		winston.warn('[plugins/load] The following plugins may not be compatible with your version of QueryTower. This may cause unintended behaviour or crashing. In the event of an unresponsive QueryTower caused by this plugin, run `./QueryTower reset -p PLUGINNAME` to disable it.');
